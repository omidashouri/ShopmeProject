...
function makeAjaxRequestPost() {
    ...
    let username = "omidashouri";

    httpRequest.open("POSt", postsUrl);
    // httpRequest.send();
    httpRequest.send("username=" + encodeURIComponent(username) + "&" + "age=" + (40).toString() + "&" + "job=software engineer");
...
function ajaxResponseForPosts() {
    ...
    if (httpRequest.readyState == XMLHttpRequest.DONE) {
        ...
        if (httpRequest.status == 200) { //201 -> successfully & there is no content to display
            //success
            //get data
            ...
            alert("POST request executed successfully");
        } else {
            //failure
